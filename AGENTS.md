...
| [`docs/decisions/lot-v5-24.md`](docs/decisions/lot-v5-24.md) | **A327** — la feuille de partage dit QUOI FAIRE : l'app choisit le canal (serveur joignable → en ligne ; serveur muet + adresse locale → en direct ; sinon PAR L'ÉCRAN d'office), plus de sélecteur en tête mais une ligne d'état (mode + raison) et des étapes numérotées dont le bouton EST l'étape (par l'écran en miroir : hôte ① Montrer ② Recevoir, invité ① Recevoir ② Renvoyer, ③ refaire) ; « Envoyer le lien… » remplace l'adresse brute ; le bandeau du mode crise = l'étape ① compressée, et l'HÔTE REÇOIT aussi ; porte de secours « Mode : automatique › » (feuille à quatre rangées, Automatique réarme le retour) ; `slPhase`/`slSheetNow`, seule porte de « quelle feuille montrer » |
