...
| [`docs/decisions/lot-v5-27.md`](docs/decisions/lot-v5-27.md) | **A333-A334** — « Reprendre » après une complication RAMÈNE le passage interrompu au bout du journal (même visite, coches gardées, `navRestore`) et la carte ⚡ se range juste avant lui — décision de l'auteur qui RENVERSE A126 (« cases neuves ») ; les replis suivent leur visite (`ovFoldRemap`, aussi chez l'invité), et « l'entrée suivante du fil » saute les excursions (`navNextIdx`, pure : une décision répondue ne se rouvre pas parce qu'une carte ⚡ s'est rangée devant sa cible). **A334** : la barre « ↩ Bloc… » se resynchronise dans `render()` — le tick des minuteurs, seul à l'appeler, s'arrête avec la session, et « Terminer » la laissait sur l'accueil pour toujours (le retour d'en-tête, une seconde) |
