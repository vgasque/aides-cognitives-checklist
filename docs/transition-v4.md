...
| **T10** | Couches CSS (`@layer`) + 4 coques de fenêtres. **⛔ REFUSÉ, MESURÉ (31/07/2026)** — et la prémisse de C11 était fausse. **Les « 22 coquilles » n'existent pas** : les 22 fenêtres partagent DÉJÀ une coque unique (`.ai-modal` > `.ai-card` > `.ai-top` > `.ai-x`) avec quatre modificateurs (`.dlg-confirm`, `.sheet-live`, `.sheet-full`, `.pdf-modal`), et ne portent en tout que **23 règles CSS sur mesure** — dont **14 pour la seule fenêtre d'appariement**, qui est une surface réellement différente. **Dix-sept des vingt-deux n'ont AUCUNE règle propre.** C11 comptait des INSTANCES DE BALISAGE, pas des coquilles. Quant à `@layer` : il ajouterait un SECOND mécanisme de cascade à côté de la discipline déjà documentée (`#id` quand l'ordre déciderait — **113 règles**), avec le risque propre qu'une règle NON layerisée l'emporte sur toutes les layerisées : une adoption partielle est pire que rien. Aucun gain mesurable. | — | — |
| **T11** | Réécrire la doctrine. **◐ PARTIELLEMENT LIVRÉ** : la passe de CORRECTION est faite — **sept affirmations devenues fausses** (mode lecteur décrit comme vivant, liste des overlays, coquille du moniteur, ordre du menu ⋯, régime de refus de navigation distante, carte du monofichier) et **deux chiffres périmés** (61 → 63 bannières, ~18 500 → ~18 800 lignes) corrigés. **RESTE** : la SCISSION doctrine / journal des incidents (moitié non faite de T0) — 3 370 lignes, travail éditorial à part entière, à ne pas bâcler en fin de chantier. | après T8 | **élevé** |
| **T12** | Réécrire `AI_PROMPT`. **✅ LIVRÉ** — mais pas comme prévu : le schéma v3 à chaînes **reste valide** (`migrate` accepte les deux formes), donc rien n'était cassé. On AJOUTE la forme facultative `items` pour `level`/`memory`/`dual`, avec la consigne de NE PAS les deviner. `audit-prompt` extrait le bloc enrichi du schéma affiché et le passe par `migrate()` : 13 → 19 contrôles. | T6 | moyen |
| **T13** | Correctifs J0. **✅ LIVRÉ** — fiches d'exemple enrichies (discriminant, code, posologie, complication hors chaîne, `onDue`, ★, ×2, △ au premier écran), snackbar → bandeau système (recouvrement du CTA **60,7 % → 0 %**), écran de bienvenue réduit à 2 paragraphes. **⚠ J0-D5 : le gain de pixels annoncé n'existe pas sur téléphone** — la carte y est une feuille pleine hauteur, le CTA était déjà visible ; 6 px à 390, 1 px à 320, −25 % seulement à 1100 px. | T6 pour les fiches | faible |
