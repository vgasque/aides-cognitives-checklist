...
| **T5b** | **Fusionner la rangée de commandes et le quai** (décision D1). **⛔ BLOQUÉ SUR T8, ET C'EST MESURÉ (31/07/2026)** : les deux rangées réunies sur une ligne réclament **637 px pour 320 disponibles** (691 pour 390), largeurs de contenu prises enfant par enfant en `max-content`. La rangée de commandes seule en exige déjà **313 sur 320** — la v4.43.0 avait dû rendre 34 px par compression pour l'y faire tenir, et la v4.73.1 ajouter l'enroulement en dernier recours. **Rien à arbitrer tant que l'axe de densité (T8) n'a pas retiré le sélecteur Guidé/Statique de cette rangée** : la fusion n'est pas un choix de conception aujourd'hui, c'est une impossibilité arithmétique. | T5, **T8** | **élevé** |
| **T6** | **Modèle v4**. **◐ PARTIELLEMENT LIVRÉ** : `v3ToV4` / `v4ToV3` purs + réversibles (60 témoins, losslessness vérifiée sur un champ INCONNU dans les deux sens) et **détection du format dans `migrate()`** — donc les cinq chemins d'entrée d'un coup (règle 5). **RESTE** : le rendu, l'éditeur, l'export et le partage lisent toujours v3 ; rien ne PRODUIT encore de v4. `schema.sql` n'est pas à rejouer tant que le partage ne porte pas d'items. | T0 | **élevé** |
| **T7** | Éditeur v4 (★ mémoire, ×2 double, niveau à 3 crans, phase, excursions). **⛔ CONDITIONNÉ, ET C'EST STRUCTUREL** : `dual` et `level` sont des propriétés PAR ITEM, or le rendu lit `b.steps[]` — des chaînes adressées par leur POSITION. Les y accrocher demanderait une table indexée par position, c'est-à-dire le défaut même que T1 a rustiné et que T6 répare. **Préalable réel : achever T6 côté RENDU**, pas seulement côté format. | T6 **complet** | moyen |
| **T8** | Axe de densité + page SFAR + onglet Schéma. **◐ PARTIELLEMENT LIVRÉ** : l'axe nomme des DENSITÉS (« Un bloc » / « Toute la fiche »), le cran 3 est un conteneur à **trois onglets** réutilisant les rendus existants (Parcours = Échelle, Page = statique, Schéma = `buildFlowSVG` avec zoom, peinture d'état et navigation rebranchés), « Se repérer » quitte la rangée de commandes. **RESTE** : le cran « Une étape » (le lecteur est encore une SURFACE) et la page SFAR comme document distinct. **⚠ NE DÉBLOQUE PAS T5b** : rangée 313 → 271 px à 320, mais le quai en réclame 324 — 595 pour 320. | T5, T6 | **élevé** |
| **T9** | **R4** : bibliothèque unique, type en filtre, provenance sur la rangée. **Seul**, parce que c'est la rupture la plus visible. | T8 | moyen |
