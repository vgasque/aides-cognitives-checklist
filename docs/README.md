...
| A377-A379 | `lot-v5-31.md` | Minuteurs et compteurs LIÉS aux étapes : la coche lance (`starts`), la coche compte (`counts`), le bloc minute à chaque entrée (`timer`, avec la session au bloc de départ) ; effets dans le geste local seulement ; annulable par la case (10 s pour un minuteur), arrêt à la sortie de boucle, rien n'est choisi à l'échéance ; une légende d'une ligne (gris au repos, bleu sans fond en cours, pastille ambre à l'échéance, 24 px réservés) ; prompt IA : 0 à 3 liens par fiche ; micro-mouvements logiques de la légende (monter / remplir / redescendre, jauge des 10 s), rien sous mouvement réduit (A378) ; un délai repart au geste, jamais à la sonnerie (A379) |
