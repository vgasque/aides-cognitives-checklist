...
| **Catégories de données** | Adresse e-mail professionnelle (compte) ; contenu des fiches rédigées ; préférences (thème, épingles, vocabulaire personnel du journal) ; horodatages de sessions. **Si l'option d'historique synchronisé est activée** : le compte rendu des sessions **terminées** (étapes cochées, minuteurs, compteurs, repères horodatés, libellés que l'utilisateur a lui-même écrits) — **sauf la trace de vérification**, qui reste sur l'appareil d'origine, et jamais une session en cours. **Pour une session partagée** : identifiant opaque de participant, rôle déclaré, horodatages des gestes, références d'étapes / minuteurs / compteurs, projection de la fiche (cf. § 3.1). **Aucune donnée identifiante de patient.** |
...
| **Durée de conservation** | Comptes : tant qu'actif ; suppression à la demande (fonction intégrée « Supprimer mon compte »). Fiches partagées : durée d'exploitation de la bibliothèque. **Sessions : sur l'appareil, sans durée fixée par l'application** (l'utilisateur les supprime depuis l'historique) — et, **si l'utilisateur active l'option** (défaut fermé), les sessions **terminées** sont aussi conservées sur le serveur de synchronisation, sans durée fixée, jusqu'à suppression par l'utilisateur ou du compte. **Session partagée sur le serveur : purgée automatiquement 30 min après son expiration** (§ 3.1) — c'est un relais, pas un entrepôt. |
...
|---|---|---|
| **La projection de la fiche** | **11 champs, liste blanche appliquée par le serveur** : `id`, `title`, `discriminant`, `code`, `status`, `validatedAt`, `blocks` (images retirées), `start`, `timers`, `counters`, `items` | L'invité doit voir la checklist. Sont **retirés** : les images (jusqu'à plusieurs Mo de base64), les documents joints, les références, et surtout `localInfo` — pré-rempli « Tél renfort / Tél régulation ». Une fiche au statut **brouillon** ne peut pas être partagée. |
...
compte-rendu ; si internet tombe, le partage bascule seul sur ce canal. **Depuis la v5.23.2**, si cette bascule a été causée par une panne, l'appareil de l'hôte revient seul en ligne dès que le serveur répond de nouveau (trois sondes de joignabilité consécutives et au moins soixante secondes passées en direct), par le même billet d'admission remis à chaque invité par le canal chiffré ; rien de nouveau ne sort de l'appareil, et un mode choisi à la main n'est jamais contredit. Les bascules CHOISIES
